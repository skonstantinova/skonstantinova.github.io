class TodoList {
	...
	loadTasks(){
		...
		console.log("load", this.tasksData)
		const PRIORITY_LABELS = ["high", "normal", "low"];
			if(this.tasksData){
				for(let currentTask in this.tasksData){				
					let currentTaskTemplate =this.taskTemplate.cloneNode();
...
class="task-priority">${PRIORITY_LABELS[this.tasksData[currentTask].priority]}</div>`;
				...
				let statusControlTemplate = this.statusControlTemplate.outerHTML;
...
manageTask(event) {
		...
		let currentAction = event.target.value
			,currentTaskId = event.target.parentElement.id;
		this[currentAction](currentTaskId);
	}

	filterTasksByTitle(event){
		if(event.keyCode == 13){
			let filterTitle = event.target.value
	...
	filterTasksByStatus(event) {
		console.log(event);
		let filterPriority = event.target.value
		 	,tasks = document.querySelectorAll(".task");
		 	for(let i = 1; i < tasks.length; i++){
	...
	clearForm(){
		let createFormItems = this.createForm.childNodes;
	...
	createTodoItem(event) {
		...
		let currentTaskJSON = this.tasksData[currentTaskTemplate.id];

		for(let i = 0 ; i < createFormItems.length; i ++){
			if(createFormItems[i].tagName === "INPUT" || createFormItems[i].tagName ===
		"SELECT"){ currentTaskTemplate.innerHTML += ` <div
		...
		this.tasks.appendChild(currentTaskTemplate);
		let selects = document.querySelectorAll(".task-status-control");
	...
	editTodoItem(sCurrentTaskId){
		let currentTask = document.getElementById(sCurrentTaskId)
			,createFormItems = this.createForm.childNodes
			,className = ""
			,taskKey = ""
			;
			//console.log(this.tasksData);
	...
	const PRIORITY_LABELS = ["high", "normal", "low"];
		this.getFromLocalStorage();		
		let currentTaskData = this.tasksData[sCurrentTaskId]
			,createFormItems = oCreateFormItems
			,className = ""
			,taskKey = ""
			;
